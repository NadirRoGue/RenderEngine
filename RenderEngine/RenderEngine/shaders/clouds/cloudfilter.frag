...
#ifdef BIG_KERNEL
// Big filter: -slower, +smoother result
#define maskSize 25u
...
#else
// Small filter: +faster, -too many high frecuency artifacts
#define maskSize 9u
...
vec4 getCloudInfo(vec2 modUV)
{
	...
	// Temporal reprojection access check. Access either texture depending on the coordinate we are treating
	int coordIter = x % 2;
...
void main()
{
	...
	vec3 gr = color.rgb * (1.0 - color.a) * 2.0;
